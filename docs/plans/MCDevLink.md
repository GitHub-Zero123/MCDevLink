...
TCP 服务默认绑定 `127.0.0.1` 和系统分配的临时端口，避免调试协议在无明确需求时暴露到外部网络。跨设备调试应显式绑定宿主的具体局域网 IPv4；只有确需监听全部 IPv4 网卡时才显式配置 `0.0.0.0`。监听地址与 discovery 中告知客户端的地址是两项独立配置。

未显式配置 discovery 目标时枚举本机 IPv4，并向各地址的 Safaia 端口范围单播。原因是 Minecraft 可能把 UDP socket 绑定到虚拟网卡地址，单发 `127.0.0.1` 无法到达；这项自动枚举不得改变 TCP 的环回监听默认值。
...
公共事件中的文本统一使用 UTF-8；Windows 系统错误等本地代码页文本必须在进入 `DiagnosticEvent` 前完成转换。
