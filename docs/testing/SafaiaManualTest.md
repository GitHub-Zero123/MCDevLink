...
程序在 Windows 启动时会将控制台输入、输出代码页设置为 UTF-8；设置失败会报告 Win32 错误码并退出，避免把 socket 收到的 UTF-8 日志按系统代码页误显示。随后程序应输出 TCP 监听地址、随机监听端口和 discovery 目标 `26613..26622`。本机模式应监听 `127.0.0.1`；跨设备模式应监听命令行传入的宿主局域网 IPv4，而不是默认暴露全部网卡。保持程序运行，再启动 Minecraft 并进入能产生脚本日志的环境。
